...
function ExperienceCard({
  ...
  return (
   <div className="group relative overflow-hidden rounded-[28px] border border-(--border)
            ...
            0_18px_40px_rgba(0,0,0,0.35)] transition-[transform,border-color,box-shadow] duration-500
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          {skills.map((skill) => (
            <span key={skill} className="rounded-full border border-(--border) bg-(--hover-bg)
                                          px-4 py-2 text-sm transition-[border-color,color] duration-300 hover:border-(--accent)
